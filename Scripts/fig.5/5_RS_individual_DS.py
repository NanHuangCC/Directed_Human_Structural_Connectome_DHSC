...
if __name__ == '__main__':
    for sample in subject_list: # test 3 subjects
        ...
        sigma = np.arange(0.030, 0.0601, 0.005)

        weights = personalized_symm_directional_conn/ np.max(personalized_symm_directional_conn)
        lengths = personalized_symm_directional_conn_len.copy()
        Duration = 61 * 1000.0  # ms * 0.1
        TR = 72  # ms * 0.1
        seed = 42
        window_length = 42  # frames
        overlap = 40  # 
        output_dir = f"/data3/nh_data/20250204_DirectionalConnectome/dataset/simulation_output/20250916_HCA-YA100_individual/directed_symm/{sample}"
