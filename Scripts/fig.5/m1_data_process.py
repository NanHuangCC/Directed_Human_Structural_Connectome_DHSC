...
def getFCD(ts, window_length, overlap):
    """
    BOLD signal   ....[samples * regions]
    window length ....[samples]
    overlap       ....[samples] 
    output: 
    FCD           ....[n_windows * n_windows]
    t             ....[n_windows]
    FC_t          ....[n_regions*(n_regions-1)/2) * n_windows]
    
    """
    # overlap can't be longer or equal to the window length
    if overlap >= window_length:
        overlap = window_length-1
        
    # transpose ts if n_samples < n_regions
...
#        n_samples, n_regions = ts.shape
    ...
    t = np.linspace(start = window_length/2, stop = window_length + n_windows*window_steps_size, 
